...
It measures distance (L2 distance in our case) between output of several layers of VGG16 pretrained model
...
def perceptual_loss(input_shape=(64, 64, 3),
    ...
    def loss(y_true, y_pred):
        ...
        loss = l2_distance(y_true, y_pred)
        loss += [l2_distance(f_in, f_out) * w for f_in, f_out, w in zip(in_feat, out_feat, loss_weights)]
        return K.mean(loss)
